...
mod tests {
	...
	async fn sync_from_same_chain() {
		...
		match synchronize_listeners(&chain, Network::Bitcoin, listeners).await {
			Ok((cache, header)) => {
				...
				// Verify that the starting blocks from each listener are in the cache
				// (these are inserted when resolving the previous best block)
	...
	async fn sync_from_different_chains() {
		...
		match synchronize_listeners(&main_chain, Network::Bitcoin, listeners).await {
			Ok((cache, header)) => {
				...
				// Verify main chain blocks up to height 3 are in the cache
				// (inserted during find_difference when walking back from the target tip)
	...
	async fn sync_from_overlapping_chains() {
		...
		match synchronize_listeners(&main_chain, Network::Bitcoin, listeners).await {
			Ok((cache, header)) => {
				assert_eq!(header, main_chain.tip());
				// Verify that the common ancestor at height 1 is in the cache
				// (all three listeners share this common ancestor with main_chain)
				assert!(cache.look_up(&main_chain.at_height(1).block_hash).is_some());
			},
			Err(e) => panic!("Unexpected error: {:?}", e),
		}
