...
use crate::lsps2::client::PeerState as LSPS2ClientPeerState;
...
/// The secondary namespace under which the [`LSPS2ClientHandler`] data will be persisted.
///
/// [`LSPS2ClientHandler`]: crate::lsps2::client::LSPS2ClientHandler
pub const LSPS2_CLIENT_PERSISTENCE_SECONDARY_NAMESPACE: &str = "lsps2_client";
...
pub(crate) async fn read_lsps2_client_peer_states<K: KVStore>(
	kv_store: K,
) -> Result<HashMap<PublicKey, Mutex<LSPS2ClientPeerState>>, lightning::io::Error> {
	let mut res = new_hash_map();
		...
		.await?
	{
		...
		let peer_state = LSPS2ClientPeerState::read(&mut reader).map_err(|_| {
			lightning::io::Error::new(
				lightning::io::ErrorKind::InvalidData,
				"Failed to deserialize LSPS2 client peer state",
			)
		})?;
